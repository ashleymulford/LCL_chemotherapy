...
#Make ALL column
for(tiss in tiss_list){
  arac_output <- fread("/home/ashley/LCL_chemotherapy/ALL/ALL_assoc_gemma_output/ALL_assoc_arac_" %&% tiss %&% ".assoc.txt")
  gene_count<-nrow(arac_output)
  gene_count_list4<-append(gene_count_list4, gene_count)
}

ALL_Gene_Count<-gene_count_list4

#Create data frame, four columns from lists created above, column names = list names, row names = tissues
gene_count_df<-data.frame(YRI_Gene_Count, CEU_Gene_Count, ASN_Gene_Count, ALL_Gene_Count, row.names = tiss_list)

#Output data frame into directory
fwrite(gene_count_df, "/home/ashley/LCL_chemotherapy/PrediXcan_gene_count_ALL", na = "NA", quote = F, sep = "\t", col.names = T, row.names = T) 
